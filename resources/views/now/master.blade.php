...
<meta name="csrf-token" content="{{ csrf_token() }}">
    ...
    <link href="/now/public.core.mind0fc.css" rel="stylesheet">
    <link href="/now/css/timecircles/TimeCircles.css" rel="stylesheet">
    <link href="/now/rating/star-rating.min.css" rel="stylesheet">
    <link href="/now/css/public.core.min.css" rel="stylesheet">
    <link href="/now/css/style.css" rel="stylesheet">
    ...
    <script src="/now/js/public.core.mind0fc.js?54500d" type="text/javascript"></script>
...
<body>
        ...
        <nav class="white" role="navigation" style="box-shadow:none;border-bottom: #eee 1px solid;">
            <div class="container-header clearfix">
               ...
               {{--  <div class="top-cate">
                    ...
                    @foreach(App\Http\Controllers\MasterController::getLoaicuahang() as $item)
                        <a href="/loai-cua-hang/{{$item->tenloaicuahang}}">
                            {{ $item->tenloaicuahang }}
                        </a>
                    @endforeach
    ...
    {{-- {{dd(Auth::user()->thongtincuahang)}} --}}

    <a href="javascript:void(0)" class="logined-user turn-off-overlay" data-activates="dropdown-user" onclick="onClickLogined()">

        <img src="/upload/icon/userlogin.png" style="border-radius:50%;">
        <span class="name-user bold capitalize">{{Auth::user()->name}}</span>
        <i class="fa fa-angle-down"></i>
    </a>
    <div class="sub-nav-user">
        @if(Auth::check())
            @if(!Auth::user()->thongtincuahang)
            <a href="/dangkycuahang" title="Corporate">
                <i class="ico-user ico-corporate" style="background-color: #25ad5f;"></i>
                <span class="text">Đăng ký cửa hàng</span>
...
@yield('content')
    ...
    {{-- <script type="text/javascript">
        $(document).ready(function(){
            $(".btn-adding").click(function(){
                var txttensp = $(item->tensp).val();
                $.post("chitiet.blade.php",{font11 ng-binding:txttensp},function(data))
                $("")
            });
        });
    </script>
 --}}
    ...
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyD7cdII3LvlSVbCYzBrU8zKVoR0-VDlgJU&libraries=places&callback=initMap"
        async defer></script>
</body>
